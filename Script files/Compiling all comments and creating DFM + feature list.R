...
# Setting the working directory to the directory containing CSV files
...
# Cleaning the date column to extract year
...
# Creating dfm from tokens object
docfm <- dfm(toks,
             remove_numbers = TRUE,
             remove_punct = TRUE,
             remove_symbols = TRUE,
...
# Merge the two data frames to get both total comments and covid-related comments per year
merged_counts_covid <- merge(total_count_per_year, keyword_count_covid, by = "year")

# Calculate the percentage of covid-related comments
